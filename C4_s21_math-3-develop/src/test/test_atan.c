...
START_TEST(zero) { ck_assert_ldouble_eq(s21_atan(0.0), atan(0.0)); }
END_TEST

START_TEST(minus_zero) { ck_assert_ldouble_eq(s21_atan(-0.0), atan(-0.0)); }
END_TEST
...
START_TEST(plus_inf) {
  ck_assert_ldouble_eq(s21_atan(S21_INF_POS), atan(S21_INF_POS));
}
END_TEST

START_TEST(negative_inf) {
  ck_assert_ldouble_eq(s21_atan(S21_INF_NEG), atan(S21_INF_NEG));
}
END_TEST

START_TEST(positive_normal) {
  double val = 3.56;
  ck_assert_double_eq_tol(s21_atan(val), atan(val), 0.000001);
}
END_TEST

START_TEST(positive_big) {
  double val = 1e10;
  ck_assert_ldouble_eq_tol(s21_atan(val), atan(val), 0.000001);
}
END_TEST

START_TEST(positive_little) {
  double val = 1e-21;
  ck_assert_ldouble_eq(s21_atan(val), atan(val));
}
END_TEST
...
Suite *s21_atan_cases(void) {
  Suite *suite = suite_create("s21_atan_cases");
